function [ RawData, Spectrum, WholeRawData] = OCTFileGetRawDataWhole( handle, index )
   ...
   node = thisList.DataFile{k};
    ...
    if ~isempty(node) && strcmpi(node.Attributes.Type, 'Raw')
        if ~isempty(strfind(node.Text, ['data\Spectral', int2str(index),'.data']))
            ...
            numApos = str2double(node.Attributes.ApoRegionEnd0);  %Number of apodization (background) scans (usually 25)
            ...
            WholeRawData=RawData;

            Spectrum = mean(RawData(:,1:numApos), 2);  %The background scans

            RawData = RawData(:,numApos+1:end);        %The Raw data of the sample
